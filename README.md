...
The directory *body_proportions* contains a subdirectory for each model. Each subdirectory contains a CSV file that assigns the calculated distances between the body keypoints to the corresponding image. The keypoints are each stored in a separate CSV file for each image in the directory *keypoints*.
